...
public class FileController {
    ...
    public ResponseEntity<UploadSongResponse> uploadAlbum(@RequestParam("song") MultipartFile[] uploadSongs,
                                                                @ModelAttribute("songInfos") UploadSongInfoDto[] songInfos,
                                                                @RequestParam("image") MultipartFile uploadImage,
                                                                Principal principal)  {
        String imagePath = null;
        Member currentUser = memberService.findByUserId(principal.getName().toString()).get();
        String dirPath = fileService.makedir(Arrays.asList(songInfos[0].getArtist(), songInfos[0].getAlbumName()));

        if(!uploadImage.isEmpty()) {
            imagePath = fileService.saveFile(uploadImage, dirPath);
        }

        Album albumSaved = fileService.saveAlbumInfo(currentUser, songInfos[0], imagePath);

        for(UploadSongInfoDto songInfo : songInfos) {
